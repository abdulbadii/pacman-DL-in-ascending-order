...
#include "../../lib/libalpm/alpm.h"
#include "../../lib/libalpm/alpm_list.h"
...
static int question(short preset, const char *format, va_list args)
{
	...
	if(preset==2) {
		fprintf(stream, " %s ", _("[Y/n/else key: yes in ascending size order]"));
		} else if(preset) {
	...
	if(safe_fgets_stdin(response, sizeof(response))) {
		...
		if(mbscasecmp(response, _("Y")) == 0 || mbscasecmp(response, _("YES")) == 0) {
			...
			return 0;
		} else return ASCENDING=1;
	}
	return 0;
}

int yesnoas(const char *format, ...)
{
	int ret;
	va_list args;

	va_start(args, format);
	ret = question(2, format, args);
	va_end(args);
